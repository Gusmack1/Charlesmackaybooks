...
// Generate standardized blog template
const generateStandardizedBlogTemplate = (blogId, blogData) => {
  const relatedBookIds = blogToBookMapping[blogId] || [];
...
export const metadata: Metadata = {
  title: \`${blogData.title} | Charles E. MacKay\`,
  description: '${blogData.excerpt}',
  keywords: ${JSON.stringify(blogData.tags || [])},
  openGraph: {
...
const post = {
  ...
  readingTime: ${blogData.readTime ? parseInt(blogData.readTime) : 12},
  featuredImage: {
    url: '/blog-images/${blogId}.jpg',
    alt: '${blogData.title}',
    caption: '${blogData.title} - Expert analysis by Charles E. MacKay'
  },
  category: '${blogData.category || 'Aviation History'}',
...
console.log('Standardizing blog posts...');
...
blogFolders.forEach(folder => {
  ...
  if (fs.existsSync(pagePath)) {
    try {
      ...
      // Extract blog data from current content
      const blogDataMatch = currentContent.match(/const blogData = ({[\s\S]*?});/);
      
      if (blogDataMatch) {
        // Parse the blog data (simplified approach)
        const blogDataStr = blogDataMatch[1];
        
        // Extract title
        const titleMatch = blogDataStr.match(/title:\s*[`'"]?([^`'"]+)[`'"]?/);
        const title = titleMatch ? titleMatch[1] : folder.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        
        // Extract excerpt
        const excerptMatch = blogDataStr.match(/excerpt:\s*[`'"]?([^`'"]+)[`'"]?/);
        ...
        // Extract content
        const contentMatch = blogDataStr.match(/content:\s*`([\s\S]*?)`/);
        const content = contentMatch ? contentMatch[1] : '<p>Content coming soon...</p>';
        
        // Extract other fields
        const readTimeMatch = blogDataStr.match(/readTime:\s*['"`]?(\d+)/);
        const readTime = readTimeMatch ? readTimeMatch[1] : '12';
        
        const categoryMatch = blogDataStr.match(/category:\s*['"`]?([^'"]+)['"`]?/);
        const category = categoryMatch ? categoryMatch[1] : 'Aviation History';
        
        const tagsMatch = blogDataStr.match(/tags:\s*(\[[\s\S]*?\])/);
        ...
        // Generate new standardized template
        const newContent = generateStandardizedBlogTemplate(folder, blogData);
        
        // Write new content
        fs.writeFileSync(pagePath, newContent);
        console.log(`✅ Updated: ${folder}`);
      } else {
        console.log(`⚠️  No blogData found in: ${folder}`);
      }
    } catch (error) {
...
console.log('Blog post standardization complete!'); 