...
console.log('Updating blog posts to use ComprehensiveBlogTemplate...');
...
blogFolders.forEach(folder => {
  ...
  if (fs.existsSync(pagePath)) {
    try {
      ...
      content = content.replace(
        /import BlogPostTemplate from ['"]@\/components\/BlogPostTemplate['"]/g,
        'import ComprehensiveBlogTemplate from \'@/components/ComprehensiveBlogTemplate\''
      );
      
      // Replace export function
      content = content.replace(
        /export default function BlogPost\(\) \{\s*return \(\s*<BlogPostTemplate \s*blog=\{blogData\}\s*relatedBooks=\{relatedBooks\}\s*relatedPosts=\{relatedPosts\}\s*\/>\s*\)\s*\}?/g,
        'export default function BlogPost() {\n  return <ComprehensiveBlogTemplate post={post} />\n}'
      );
      
      // Add metadata export if not present
      if (!content.includes('export const metadata')) {
        // Extract title from blogData
        const titleMatch = content.match(/title:\s*[`'"]?([^`'"]+)[`'"]?/);
        const title = titleMatch ? titleMatch[1] : folder.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
        
        // Extract excerpt from blogData
  ...
  openGraph: {
    title: \`${title}\`,
    description: '${excerpt}',
    url: 'https://charlesmackaybooks.com/blog/${folder}',
  ...
  twitter: {
    ...
    images: ['/blog-images/${folder}.jpg'],
      ...
      // Convert blogData to post format
      content = content.replace(
        /const blogData = ({[\s\S]*?});/,
        (match) => {
          ...
          const contentMatch = match.match(/content:\s*`([\s\S]*?)`/);
          const content = contentMatch ? contentMatch[1] : '<p>Content coming soon...</p>';
          
          const readTimeMatch = match.match(/readTime:\s*['"`]?(\d+)/);
...
console.log('Blog template updates complete!'); 