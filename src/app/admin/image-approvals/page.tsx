...
export default function ImageApprovalsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <p className="text-sm text-slate-600 mb-6">Review candidate images per post. Approvals will be exported as a JSON you can place in <code>src/data/image-approvals.json</code>.</p>

      <div className="flex justify-end mb-4">
        <button onClick={download} className="px-4 py-2 rounded bg-blue-600 text-white">Download Updated Manifest</button>
      </div>
          ...
          return (
            <section key={slug} className="border rounded p-4">
              <div className="flex items-center justify-between mb  -2">
                ...
                <div className="text-sm">Approved {approved}/{rec.requiredCount}</div>
              </div>
                ...
                {rec.candidates.map((c, idx) => (
                  <div key={idx} className="border rounded p-3">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <button className={`px-2 py-1 rounded ${rec.approvedFeaturedIndex === idx ? 'bg-green-600 text-white' : 'bg-slate-200'}`} onClick={() => setFeatured(slug, idx)}>
                        {rec.approvedFeaturedIndex === idx ? 'Featured' : 'Set as Featured'}
                      </button>
                    </div>
                    <div className="mb-3">
                      <label className="block text-sm mb-1">Image URL</label>
                      <input className="w-full border rounded p-2" value={c.url} onChange={e => updateField(slug, idx, 'url', e.target.value)} placeholder="https://..." />
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                      <div>
                        <label className="block text-sm mb-1">Source</label>
                        <input className="w-full border rounded p-2" value={c.source} onChange={e => updateField(slug, idx, 'source', e.target.value)} placeholder="commons.wikimedia.org/..." />
                      </div>
                      <div>
                        <label className="block text-sm mb-1">License</label>
                        <input className="w-full border rounded p-2" value={c.license} onChange={e => updateField(slug, idx, 'license', e.target.value)} placeholder="CC BY-SA 4.0 / Public Domain" />
                      </div>
                    </div>
                    <div className="mb-3">
                      <label className="block text-sm mb-1">Title</label>
                      <input className="w-full border rounded p-2" value={c.title} onChange={e => updateField(slug, idx, 'title', e.target.value)} />
                    </div>
                    <div className="mb-3">
                      <label className="block text-sm mb-1">Alt</label>
                      <input className="w-full border rounded p-2" value={c.alt} onChange={e => updateField(slug, idx, 'alt', e.target.value)} />
                    </div>
                    <div className="mb-3">
                      <label className="block text-sm mb-1">Caption</label>
                      <textarea className="w-full border rounded p-2" value={c.caption || ''} onChange={e => updateField(slug, idx, 'caption', e.target.value)} />
                    </div>
                    {c.url && (
                      <div className="aspect-video bg-slate-50 rounded overflow-hidden border">
              ...
              <div className="mt-3">
                <button onClick={() => addCandidate(slug)} className="px-3 py-2 rounded bg-slate-200">Add candidate</button>
              </div>
            </section>
          );
        })}
