...
import { useComprehensiveWebsiteFixer, ComprehensiveFixReport, WebsiteFixResult } from '../utils/comprehensiveWebsiteFixer';

export default function ComprehensiveWebsiteFixer() {
  const { runFix, report, isRunning, fixer } = useComprehensiveWebsiteFixer();
  const [currentPhase, setCurrentPhase] = useState<number>(0);
  const [phaseProgress, setPhaseProgress] = useState<Map<number, number>>(new Map());

  useEffect(() => {
    fixer.on('phase-start', (data) => {
  ...
  const handleRunFix = async () => {
    try {
      ...
      console.error('Failed to run comprehensive fix:', error);
    }
  };
