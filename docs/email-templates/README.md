...
wraps and posts every order from Glasgow. Angus runs the digital backend
...
wrap and post to Charles, never to Angus. Avoid corporate language ("we value
...
- Charles writes the books AND wraps and posts every order. Angus runs
  the digital side. Mention the Charles wrap and post somewhere, it is the
  whole point of the brand. NEVER write "I wrap" or "we wrap" in Angus's
  voice; the wrap and post is always Charles's. Use "Angus" not "Gus" in
...
- Glasgow / Scotland is the location, lean into it ("based in Glasgow",
  "wrapped and posted from Glasgow").
- No emoji.
